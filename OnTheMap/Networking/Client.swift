...
class Client: NSObject {
    ...
    func authenticateWith(userEmail: String, andPassword: String, completionHandlerForAuth: @escaping (_ success: Bool, _ errorString: String?) -> Void) {
        ...
        _ = taskForPOSTMethod(Constants.Methods.Authentication, parameters: [:], jsonBody: jsonBody, completionHandlerForPOST: { (results, error) in
            if let error = error {
                print(error)
                completionHandlerForAuth(false, "Login Failed.")
            } else {
                if let account = results?[Constants.JSONResponseKeys.Account] as? [String: AnyObject] {
                    guard let registered = account[Constants.JSONResponseKeys.Registered] as? Bool, registered == true else {
                        completionHandlerForAuth(false, "Login Failed, user not registered.")
                        return
                    }
                    guard let session = results?[Constants.JSONResponseKeys.Session] as? [String: AnyObject] else {
